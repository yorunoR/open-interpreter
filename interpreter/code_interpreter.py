...
class CodeInterpreter:
    ...
    def start_process(self):
        ...
        self.proc = subprocess.Popen(start_cmd.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=0)

        # Start watching ^ its `stdout` and `stderr` streams
        threading.Thread(target=self.save_and_display_stream, args=(self.proc.stdout, False), daemon=True).start()  # Passes False to is_error_stream
        threading.Thread(target=self.save_and_display_stream, args=(self.proc.stderr, True), daemon=True).start()  # Passes True to is_error_stream
        ...
        handlers=[
            ast.ExceptHandler(
                ...
                body=[
                    ast.Expr(
                        value=ast.Call(
                            func=ast.Attribute(value=ast.Name(id="traceback", ctx=ast.Load()), attr="print_exc", ctx=ast.Load()), args=[], keywords=[]
                        )
                    ),
                ],
            )
